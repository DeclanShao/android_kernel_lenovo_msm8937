...
static u8 g_proximity_en = 0;
	...
	{
		#include "Lenovo_3_b_b.i" 
	};

	static unsigned char i_CTPM_FW_2[]=
	{
		#include "Lenovo_INX_5_10_4.i" 
	};
...
struct himax_i2c_platform_data *lct_pdata;

extern int panel_id_ret;
...
static int i_update_FW(void)
{
	...
	unsigned char* ImageBuffer_2 = i_CTPM_FW_2;
	int fullFileLength = sizeof(i_CTPM_FW); 
	int fullFileLength_2 = sizeof(i_CTPM_FW_2); 
	//I("IMAGE FW_VER=%x,%x.\n",i_CTPM_FW[FW_VER_MAJ_FLASH_ADDR],i_CTPM_FW[FW_VER_MIN_FLASH_ADDR]);
	//I("IMAGE CFG_VER=%x.\n",i_CTPM_FW[FW_CFG_VER_FLASH_ADDR]);
	//I("IMAGE FW_VER=%x,%x.\n",i_CTPM_FW_2[FW_VER_MAJ_FLASH_ADDR],i_CTPM_FW_2[FW_VER_MIN_FLASH_ADDR]);
	//I("IMAGE CFG_VER=%x.\n",i_CTPM_FW_2[FW_CFG_VER_FLASH_ADDR]);
#ifdef HX_RST_PIN_FUNC
	//ESD_HW_REST();
	himax_HW_reset(true,true);
#endif
	i2c_himax_write_command(i2c_client_point, 0x82, DEFAULT_RETRY_CNT);
	msleep(50);

	i2c_himax_write_command(i2c_client_point, 0x80, DEFAULT_RETRY_CNT);
	msleep(50);
	if (1 == panel_id_ret)
	{
		if(( private_ts->vendor_fw_ver_H < i_CTPM_FW_2[FW_VER_MAJ_FLASH_ADDR] )
			|| ( private_ts->vendor_fw_ver_L < i_CTPM_FW_2[FW_VER_MIN_FLASH_ADDR] )
			|| ( private_ts->vendor_config_ver < i_CTPM_FW_2[FW_CFG_VER_FLASH_ADDR])
			|| ( himax_calculateChecksum(false) == 0 ))
			{		 
#ifdef HX_RST_PIN_FUNC
				himax_HW_reset(false,true);
#endif
				if(fts_ctpm_fw_upgrade_with_fs(ImageBuffer_2,fullFileLength_2,true) == 0)
					E("%s: TP upgrade error\n", __func__);
				else
					{
						I("%s: TP upgrade OK\n", __func__);
						private_ts->vendor_fw_ver_H = i_CTPM_FW_2[FW_VER_MAJ_FLASH_ADDR];
						private_ts->vendor_fw_ver_L = i_CTPM_FW_2[FW_VER_MIN_FLASH_ADDR];
						private_ts->vendor_config_ver = i_CTPM_FW_2[FW_CFG_VER_FLASH_ADDR];
					}
#ifdef HX_RST_PIN_FUNC
		...
		{
#ifdef HX_RST_PIN_FUNC
			himax_HW_reset(false,true);//qijin add 
#endif
			return 0;
		}

	}else if(panel_id_ret == 0){
		...
		else
		{
            ...
            E("%s:error: no such touch screen\n", __func__);
            return 0;
        }
}
#endif
...
static void himax_set_cover_func(unsigned int enable)
{
	...
	if (enable)
		...
		cmd[0] = 0x00;
	if ( i2c_himax_write(private_ts->client, 0xF3 ,&cmd[0], 1, DEFAULT_RETRY_CNT) < 0)
		E("%s i2c write fail.\n",__func__);
...
static int himax_parse_wake_event(struct himax_ts_data *ts)
{
	...
	I("get ID value = %x,%x,%x,%x,%x,%x,%x,%x of thi event\n",
...
bypass_checksum_failed_packet:
	...
	if(ts->pdata->proximity_bytp_enable){
			if ( (buf[HX_TOUCH_INFO_POINT_CNT] & 0x0F)  == 0x00 && (buf[HX_TOUCH_INFO_POINT_CNT+2]>>2 & 0x01) )
			{
				...
				__pm_wakeup_event(&ts->ts_wake_lock, TS_WAKE_LOCK_TIMEOUT);
				return;
			}
		}
#endif
	...
	if(!HX_PROC_SEND_FLAG)
		{
			...
			ret += sprintf(buf, "[vendor]%s,[FW]%#x%x%x,[IC]%s\n", TP_VENDOR_NAME,
				ts_data->vendor_fw_ver_H, ts_data->vendor_fw_ver_L, ts_data->vendor_config_ver, TP_IC_NAME);
			HX_PROC_SEND_FLAG=1;
		}
	else
		HX_PROC_SEND_FLAG=0;
	...
	size_t len, loff_t *pos)
{
	...
	else if (buf[0] == 't')
	{
		...
		himax_int_enable(private_ts->client->irq,0);
		__pm_stay_awake(&private_ts->ts_flash_wake_lock);
#ifdef HX_CHIP_STATUS_MONITOR
		HX_CHIP_POLLING_COUNT = 0;
	...
	himax_HW_reset(true,false);
#endif
	__pm_relax(&private_ts->ts_flash_wake_lock);
	himax_int_enable(private_ts->client->irq,1);
#ifdef HX_CHIP_STATUS_MONITOR
	HX_CHIP_POLLING_COUNT = 0;
...
size_t count_selftest = 0;
#endif
static int himax_chip_self_test(char *buf)
{
	int pf_value=0x00;
#ifdef HIMAX_BANK_SELF_TEST
	...
	//open sorting mode
	cmdbuf[0] = 0x80;
		i2c_himax_write(private_ts->client, 0xF4,&cmdbuf[0], 1, DEFAULT_RETRY_CNT);
		msleep(120);
	...
	//Write Criteria
		...
		msleep(10);
		i2c_himax_write(private_ts->client, 0x40, &test_criteria_data[0], 6, DEFAULT_RETRY_CNT);
		...
		count_selftest = count;
		//I("[Himax]: himax_chip_self_test count_selftest = %d \n", count_selftest);
		msleep(120);
		...
		i2c_himax_read(private_ts->client, 0x5A, databuf, 7, DEFAULT_RETRY_CNT);

		cmdbuf[0] = 0x00;
		i2c_himax_write(private_ts->client, 0x8C ,&cmdbuf[0], 1, DEFAULT_RETRY_CNT);

	for(i=0;i<7;i++) {
		I("[Himax]: After slf test %X databuf[%d] = 0x%x\n", addr_result,i,databuf[i]);
	}
	diag_command = 0;
	if (databuf[0]==0xAA) {
...
#endif
#ifdef GLOVE
static ssize_t himax_glove_read(struct file *file, char *buf,
	size_t len, loff_t *pos)
{
	struct himax_ts_data *ts = private_ts;
	size_t ret = 0;
...
static ssize_t himax_glove_write(struct file *file, const char *buff,
	size_t len, loff_t *pos)
{
	struct himax_ts_data *ts = private_ts;
	char buf[80] = {0};
...
static struct file_operations himax_proc_glove_ops =
{
	.owner = THIS_MODULE,
	.read = himax_glove_read,
	.write = himax_glove_write,
};
#endif
...
static int himax_touch_proc_init(void)
{
...
#ifdef GLOVE
	himax_proc_glove_file = proc_create(HIMAX_PROC_GLOVE_FILE, (S_IWUSR|S_IRUGO|S_IWUGO),
		himax_touch_proc_dir, &himax_proc_glove_ops);
	if(himax_proc_glove_file == NULL)
	{
		E(" %s: proc glove file create failed!\n", __func__);
		goto fail_13;
	}
#endif
	...
	return 0 ;
#ifdef HX_SMART_WAKEUP
	fail_14: remove_proc_entry( HIMAX_PROC_SMWP_FILE, himax_touch_proc_dir );
#endif
#ifdef GLOVE
	fail_13: remove_proc_entry( HIMAX_PROC_GLOVE_FILE, himax_touch_proc_dir );
#endif
#ifdef HX_TP_PROC_HITOUCH
	fail_12: remove_proc_entry( HIMAX_PROC_HITOUCH_FILE, himax_touch_proc_dir );
#endif
#ifdef HX_TP_PROC_SELF_TEST
...
#endif
#ifdef GLOVE
		remove_proc_entry( HIMAX_PROC_GLOVE_FILE, himax_touch_proc_dir );
#endif
#ifdef HX_TP_PROC_HITOUCH
...
static int himax_parse_config(struct himax_ts_data *ts, struct himax_config *pdata)
{
	...
	if (i <= cnt) {
		...
		I(" DT#%d-def_cfg:%d,id:%05x, FW:%x, len:%d,", i,
			cfg_table[i].default_cfg, cfg_table[i].sensor_id,
			cfg_table[i].fw_ver, cfg_table[i].length);
				...
				struct himax_i2c_platform_data *pdata)
{
	...
	//pdata->abs_x_max =1680;//test
	//pdata->abs_y_max =2880;//test
	...
	//pdata->screenWidth  = 1920;//test
	//pdata->screenHeight = 1080;//test
...
static int himax_pinctrl_init(struct himax_i2c_platform_data *pdata)
{
	int retval;
	
	/* Get pinctrl if target uses pinctrl */
	pdata->ts_pinctrl = devm_pinctrl_get(&(i2c_client_point->dev));
	if (IS_ERR_OR_NULL(pdata->ts_pinctrl)) {
		retval = PTR_ERR(pdata->ts_pinctrl);
		dev_dbg(&i2c_client_point->dev,
	...
	pdata->pinctrl_state_release
		= pinctrl_lookup_state(pdata->ts_pinctrl,
			PINCTRL_STATE_RELEASE);
	if (IS_ERR_OR_NULL(pdata->pinctrl_state_release)) {
		retval = PTR_ERR(pdata->pinctrl_state_release);
		dev_dbg(&(i2c_client_point->dev),
			"Can not lookup %s pinstate %d\n",
			PINCTRL_STATE_RELEASE, retval);
	}
	pinctrl_select_state(pdata->ts_pinctrl,
				pdata->pinctrl_state_active);
err_pinctrl_get:
err_pinctrl_lookup:	
	return 0;
...
static int himax852xf_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ts->pdata->abs_x_max = 1200;
	ts->pdata->abs_y_max = 1920;
	...
	wakeup_source_init(&ts->ts_wake_lock, HIMAX852xf_NAME);
#endif
#ifdef HX_SMART_WAKEUP
	ts->SMWP_enable=0;
	wakeup_source_init(&ts->ts_SMWP_wake_lock, HIMAX852xf_NAME);
#endif
wakeup_source_init(&ts->ts_flash_wake_lock, HIMAX852xf_NAME);
...
#if defined(CONFIG_TOUCHSCREEN_PROXIMITY)
	if(pdata->proximity_bytp_enable)
		wakeup_source_trash(&ts->ts_wake_lock);
#endif
#ifdef  HX_CHIP_STATUS_MONITOR
...
#endif
	pinctrl_select_state(ts->pdata->ts_pinctrl,
					ts->pdata->pinctrl_state_release);
...
if(HX_ON_HAND_SHAKING)//chip on hand shaking,wait hand shaking
{
	...
	if (ts->pdata->powerOff3V3 && ts->pdata->power)
		ts->pdata->power(0);
	pinctrl_select_state(ts->pdata->ts_pinctrl,
				ts->pdata->pinctrl_state_suspend);
...
#endif
		//for esd
		RF6H[0]=0x01;RF6H[1]=0x80;RF6H[2]=0x00;RF6H[3]=0x00;RF6H[4]=0x00;
		RF6H[5]=0x00;RF6H[6]=0x00;RF6H[7]=0x00;RF6H[8]=0x00;RF6H[9]=0x00;
		ret_esd = i2c_himax_write(ts->client,0xf6, &RF6H[0], 10, DEFAULT_RETRY_CNT);
		if (ret_esd < 0)
		{
			E("[himax] %s: I2C access failed addr = 0x%x\n", __func__, ts->client->addr);
		}
...
#ifdef MTK
void himax852xf_suspend(struct early_suspend *h)
{
	int ret;
	uint8_t buf[2] = {0};
#ifdef HX_CHIP_STATUS_MONITOR
...
static void himax852xf_resume(struct early_suspend *h)
{
#ifdef HX_SMART_WAKEUP
	int ret;
	uint8_t buf[2] = {0};
...
#ifdef QCT
static const struct dev_pm_ops himax852xf_pm_ops = {
#if (!defined(CONFIG_FB) && !defined(CONFIG_HAS_EARLYSUSPEND))
	.suspend = himax852xf_suspend,
	.resume  = himax852xf_resume,
#else
	.suspend = himax852xf_suspend,
#endif
};
...
#ifdef MTK
static struct i2c_board_info __initdata himax852xf_i2c = { I2C_BOARD_INFO("himax852xf_NAME", (0x90 >> 1))};
static struct i2c_driver tpd_i2c_driver =
{
    .probe = himax852xf_probe,
    .remove = himax852xf_remove,
    .detect = himax852xf_detect,
    .driver.name = "himax852xf_NAME",
    .id_table = himax852xf_ts_id,
    .address_list = (const unsigned short *) forces,
};

static int himax852xf_local_init(void)
{
...
static int himax852xf_detect (struct i2c_client *client, struct i2c_board_info *info)
{
...
{
    .tpd_device_name = "himax852xf_NAME",
    .tpd_local_init = himax852xf_local_init,
    .suspend = himax852xf_suspend,
    .resume = himax852xf_resume,
...
static void __init himax852xf_init_async(void *unused, async_cookie_t cookie)
{
	...
	i2c_register_board_info(TPD_I2C_NUMBER, &himax852xf_i2c, 1);
...
{
	I("Himax 852xf touch panel driver init\n");
